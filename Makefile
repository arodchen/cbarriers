...
help :
	...
	@echo ""
	@echo ""
	@echo "Available arguments [name={valid values} (default value) - description]:"
	@echo ""                                 
	@echo "  EXTRACFLAGS={...} ( ) - extra CFLAGS"
	@echo "  EXTRALDFLAGS={...} ( ) - extra LDFLAGS"
	@echo ""                                 
	@echo "  TEST_RUNNER={...} ( ) - test runner"
	@echo "  TEST_RUNNER_PASS_ARG_BEG={...} ( ) - option to begin passing arguments to test runner"
	@echo "  TEST_RUNNER_PASS_ARG_END={...} ( ) - option to end passing arguments to test runner"
	@echo "  TEST_RUNNER_INIT={...} ( ) - option to initialize test runner"
	@echo "  TEST_RUNNER_FINI={...} ( ) - option to finalize test runner"
	@echo ""                                 
	@echo "  CPUS_NUM={1..N} (shell nproc) - number of available CPUs"
	...
	@echo "                                                        (THREADS_SURPLUS has priority over THREADS_MAX_NUM if both are defined)"
	@echo "  THREADS_MIN_NUM={1..THREADS_MAX_NUM} (1) - minimum number of threads"
	@echo "  BARRIERS_NUM={1..N} (1000000) - number of barriers in the experiment"
	...
	@echo "  THREADS_INC={+=1,*=2,...} (+= 1) - compound assignment operator for the increment of the number of threads"
	@echo "  RADIX_INC={+=1,*=2,...} (+= 1) - compound assignment operator for the increment of radix"
	@echo "  RADIX_MAX={2..THREADS_MAX_NUM} (THREADS_MAX_NUM) - maximum radix"
	@echo "  OMP_IMPL={gnu,intel} (gnu) - OMP library implementation"
	@echo "  INTERPOLATE_RADIX={yes,no} (no) - when the number of threads participating in the barrier is less than radix then the result is"
	@echo "                                    taken from the experiment when the number of threads participating in a barrier is equal to radix"
	@echo "  PRINT_SYNCH_UNSYNCH_PHASE_TIME={yes,no} (yes) - printing synchronized and unsynchronized phase execution time,"
	@echo "                                                  where the phase is an interval by two adjacent barriers"
